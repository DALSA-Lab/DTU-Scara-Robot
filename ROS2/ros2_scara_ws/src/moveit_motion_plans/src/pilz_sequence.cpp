...
int main(int argc, char** argv)
{
  ...
  auto moveit_visual_tools = moveit_visual_tools::MoveItVisualTools{ node, "base_link", "motion_plan",
  ...
  item1.req.goal_constraints.push_back(
      kinematic_constraints::constructGoalConstraints(PLANNING_FRAME, target_pose_item1));
  ...
  item2.req.goal_constraints.push_back(
      kinematic_constraints::constructGoalConstraints(PLANNING_FRAME, target_pose_item2));
  ...
  auto const draw_trajectory_tool_path =
      [&moveit_visual_tools,
       robot = move_group_interface.getRobotModel()](auto const& trajectories) {
  ...
  if (service_response->response.error_code.val == moveit_msgs::msg::MoveItErrorCodes::SUCCESS)
  {
    ...
    RCLCPP_INFO(LOGGER, "Planning took %f seconds",service_response->response.planning_time);
    ...
    display_trajectory.trajectory_start = service_response->response.sequence_start;
