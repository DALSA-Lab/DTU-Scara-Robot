...
namespace bioscara_hardware_interfaces
{
      ...
      const hardware_interface::HardwareComponentInterfaceParams &params)
  {
    ...
    /**
     * Loop over all joints decribed in the hardware description file, check if they have the position command
     * interface defined and finally add them to the internal _joints list
     *
     * @todo find out if mimic_joints appear in info_.joints
     *
     */
    for (const hardware_interface::ComponentInfo &joint : info_.joints)
    {
      ...
      {
        RCLCPP_WARN(
            get_logger(), "Joint '%s' has %zu state interface. 0 expected.", joint.name.c_str(),
    ...
    // {
    //   RCLCPP_FATAL(
    //       get_logger(), "BioscaraGripperHardwareInterface has '%ld' GPIO components, '%lu' expected. Every joint needs a GPIO interface",
  ...
  hardware_interface::return_type BioscaraGripperHardwareInterface::prepare_command_mode_switch(
  ...
  bioscara_hardware_driver::err_type_t BioscaraGripperHardwareInterface::start_homing(const std::string name, float velocity)
  {
    joint_config_t cfg = _joint_cfg[name];
  ...
  bioscara_hardware_driver::err_type_t BioscaraGripperHardwareInterface::stop_homing(const std::string name)
  {
    joint_config_t cfg = _joint_cfg[name];
